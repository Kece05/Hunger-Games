...
class GameScen: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        var diction = UserDefaults.standard.dictionary(forKey: "otherNames")
        
        for i in UserDefaults.standard.dictionary(forKey: "otherNames") as! Dictionary<String, String> {
            let OnlineSprite = taskBot.copy() as! SKSpriteNode
            OnlineSprite.texture = SKTexture(imageNamed: i.value)
            OnlineSprite.size = SKTexture(imageNamed: i.value).size()
            if OnlineSprite.texture!.name == "mountainC" {
                OnlineSprite.scale(to: CGSize(width: OnlineSprite.size.width/6, height: OnlineSprite.size.height/6))
                for i in 1...3 {
                    if let childp = OnlineSprite.childNode(withName: String(i)) as? SKSpriteNode {
                        if i == 1 {
                            childp.position = CGPoint(x: 160, y: 340.5)
                            childp.size = CGSize(width: childp.size.width*1.5, height: childp.size.height*1.5)
                        } else if i == 2 {
            ...
            } else if OnlineSprite.texture!.name == "racerC" {
                OnlineSprite.scale(to: CGSize(width: OnlineSprite.size.width/4, height: OnlineSprite.size.height/4))
                for i in 1...3 {
                    if let childp = OnlineSprite.childNode(withName: String(i)) as? SKSpriteNode {
                        var game = String()
                        if i == 1 {
                ...
                OnlineSprite.scale(to: CGSize(width: OnlineSprite.size.width/3, height: OnlineSprite.size.height/3))
            }
            ...
            OnlineSprite.name = i.key
            otherPeople.append(OnlineSprite)
            self.addChild(OnlineSprite)
        }
        taskBot.alpha = 0.0
        taskBot.zPosition = -1
...
//    }
    ...
    @objc func revicedMode(notification:NSNotification) {
        do {
            let message = try JSONDecoder().decode(work.self, from: notification.object as! Data)
            print(message)
            var x = CGFloat(message.px)
            var y = CGFloat(message.py)
            
            otherPeople[0].position = CGPoint(x: x, y: y)
            otherPeople[0].zRotation = message.r
            } catch {
                print("Failed")
            }
    }
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if move {
            ...
            do {
                ...
                let message = work(px: Int16(x), py:Int16(y), r: ply!.zRotation)
