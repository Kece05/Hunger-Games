...
class Final4everViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate, MCBrowserViewControllerDelegate, GADBannerViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        bannerView.adSize = GADCurrentOrientationAnchoredAdaptiveBannerAdSizeWithWidth(viewss.frame.size.width/2)
        ...
        bannerView.layer.zPosition = 2
        bannerView.alpha = 1
        size.delegate = self
        appDelegate.MCPHandler.session.disconnect()
        modes.layer.zPosition = 3
        ...
        if UserDefaults.standard.string(forKey: "name") == nil {
            UserDefaults.standard.set("bumbleC" ,forKey: "name")
            image.image = UIImage(named: "bumbleC")
        } else {
