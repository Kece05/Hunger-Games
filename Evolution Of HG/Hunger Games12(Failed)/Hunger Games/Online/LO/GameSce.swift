...
class GameSce: SKScene, SKPhysicsContactDelegate, StreamDelegate {
    ...
    override func didMove(to view: SKView) {
        if end {
            print("htoishiodjpjiofjasiodj")
            theWinner = UserDefaults.standard.string(forKey: "that")!
            luckyOne = UserDefaults.standard.bool(forKey: "player")
            ...
            if firstime {
                ...
                taskBot.alpha = 0.0
                taskBot.zPosition = -1
                speared.position = CGPoint(x:10000,y:10000)
                ...
                if !luckyOne {
                    for _ in 1...1 {
                        ...
                        self.addChild(spear)
                        spear.name = "\(theWinner)"
                        spearO[theWinner] = spear
                ...
                killLabel.text = "Game Mode: Lucky One"
    ...
    @objc func countdown() {
        if end {
            if time >= 74 {
                time -= 1
                killLabel.text = "Damage Active: \(time-71)"
            } else if time != 0 {
                time -= 1
                killLabel.text = fun(time: TimeInterval(time))
                if !luckyOne && ((others[theWinner]?.hasActions()) != nil) {
                    others[theWinner]?.run(SKAction.fadeAlpha(to: 0.0, duration: 3), completion: {
                        SKAction.fadeAlpha(to: 0.0, duration: 1.5)
                    })
                } else if luckyOne && ply!.hasActions() {
                    ply!.run(SKAction.fadeAlpha(to: 0.0, duration: 3), completion: {
                        SKAction.fadeAlpha(to: 0.0, duration: 1.5)
                    })
                }
            } else if time == 0  {
                home(winner: "Unluckies")
            }
        }
    }
    ...
    func NoHealth(kill:String) {
        ...
        do {
            if playersAlive == 1 {
                ...
                livers = livers.filter { $0 != appDelegate.MCPHandler.peerID!.displayName }
                if livers[0] == theWinner {
                    home(winner: theWinner)
                } else {
                    home(winner: "Unluckies")
                }
            } else {
            ...
            do {
                ...
                if message.endGame {
                    ...
                    if message.NoHealth {
                        ...
                        livers = livers.filter { $0 != message.num }
                        
                    } else {
                        if luckyOne || message.num != theWinner {
    ...
    override func update(_ currentTime: TimeInterval) {
        if end {
            ...
            if !((oneSide.x <= ply!.position.x && oneSide.y <= ply!.position.y) && (twoSide.x >= ply!.position.x && twoSide.y >= ply!.position.y)) && time <= 72 {
                health -= 3
                healthBarT.size = CGSize(width: CGFloat(health*2), height: healthBarT.size.height)
                healthBarT.position.x = ply!.position.x + CGFloat(healthBarB.size.width-CGFloat((200-health))) - 400
                if health <= 0 && alive {
