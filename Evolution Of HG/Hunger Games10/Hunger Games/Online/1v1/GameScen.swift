    ...
    override func didMove(to view: SKView) {
        if end {
            ...
            if firstime {
                ...
                self.ply!.name = "ply"
                ply!.texture = SKTexture(imageNamed: "bumbleC")
                ply!.size = SKTexture(imageNamed: "bumbleC").size()
                self.anchorPoint = .zero
                physicsWorld.speed = 0.9
                ...
                for i in names {
                    ...
                    OnlineSprite.texture = SKTexture(imageNamed: "bumbleC")
                    OnlineSprite.size = SKTexture(imageNamed: "bumbleC").size()
                ...
                taskBot.alpha = 0.0
                taskBot.zPosition = -1
                speared.position = CGPoint(x:Int.random(in: 1000...5000),y:Int.random(in: 1000...5000))
    ...
    func NoHealth(kill:String) {
        ...
        do {
            if playersAlive == 1 {
                ...
                UserDefaults.standard.set(false, forKey: "wonned")
            ...
            if time <= 175 {
                ...
                if (bodya == ply!.name && bodyb != "bot") || (bodyb == ply!.name && bodya != "bot") && spearO[theOne] != nil {
                    ...
                    spearO[theOne]!.run(SKAction.scale(to: CGSize(width: 0, height: 0), duration: 0.2), completion: {
                        self.spearO[theOne]!.position = CGPoint(x:1000,y:1000)
                    ...
                    speared.run(SKAction.scale(to: CGSize(width: 0, height: 0), duration: 0.2), completion: {
                        self.speared.position = CGPoint(x:1000,y:1000)
